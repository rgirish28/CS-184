...
void display() 
{
  ...
  if (numused) {
    ...
    // Remember that light positions must be transformed by modelview.  
		...
		for (int i=0;i<numused;i++){
			...
			GLfloat input[4],output[4];
			for(int j=0;j<4;j++){
				input[j] = lightposn[4*i+j];
			}
			transformvec(input,output);
			for(int j=0;j<4;j++){
				lightransf[4*i+j] = output[j];
			}
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    ...
    mat4 objTransf = transf*obj->transform;
    //transf = mv*sc*tr*obj->transform;
    glLoadMatrixf(&objTransf[0][0]);
